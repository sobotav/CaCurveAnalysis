...
for i = 1:Nfigures
    ...
    for k = 1:NgraphsPerFigure
        ...
        if s <= NsignalsTotal
            ...
            results(k,3) = resultsSignal.medianBeforePeak;      % median fluorescence before the start of the peak
            results(k,4) = resultsSignal.tMax1;                 % time of the first maximum
            results(k,5) = resultsSignal.peakFluorescence;      % peak fluorescence
            results(k,6) = resultsSignal.slope1;                % first slope
            results(k,7) = resultsSignal.slope1Duration;        % duration of the first slope
            results(k,8) = resultsSignal.slope2;                % second slope
            results(k,9) = resultsSignal.slope2Duration;        % duration of the second slope
            results(k,10) = resultsSignal.slope3;               % third slope
            results(k,11) = resultsSignal.slope3Duration;       % duration of the third slope
...
function results = analyzeSignal(timeStamp, signal, inputParams)
    ...
    medianBeforePeak            = calcPercentileFromInterval(timeStamp, signal, [0,inputParams.peakStart], 50);             % median value before the signal takes off
    ...
    % addition
    startLevel  = calcPercentileFromInterval(timeStamp, signal, [0,inputParams.peakStart], 95);
    ...
    % -------------------------
    % save the parameters
    results.medianBeforePeak    = medianBeforePeak;                 % median fluorescence before the start of the peak
